...
grep -q 'uses: ./.github/workflows/native-speech.yml' "$CI_WORKFLOW_PATH"
grep -q 'LIVENOTES_UI_MIN_VIDEO_SECONDS: "0"' "$CI_WORKFLOW_PATH"
grep -q 'MACOSX_DEPLOYMENT_TARGET = 26.0' "$ROOT_DIR/LiveNotes.xcodeproj/project.pbxproj"
...
grep -q 'uses: ./.github/workflows/native-speech.yml' "$WORKFLOW_PATH"
grep -q 'needs: native-speech' "$WORKFLOW_PATH"
grep -q './scripts/run-loopback-e2e-test.sh' "$ROOT_DIR/.github/workflows/native-speech.yml"
