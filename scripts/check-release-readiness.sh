...
require_grep 'MACOSX_DEPLOYMENT_TARGET = 26.0' "$PROJECT_FILE" "release builds must target macOS 26 or newer."
...
require_grep 'if #available(macOS 26[.]4, [*])' "$ROOT_DIR/LiveNotesApp/ContentView.swift" "low-latency Apple Translation UI must be runtime-gated."
require_grep 'if #available(macOS 26[.]4, [*])' "$APP_MODEL_FILE" "low-latency Apple Translation preflight must be runtime-gated."
